...
holds the key). In that arrangement `.sg_vault/publish/` is inside git's view. The folder is
**derived output** — regenerated by every publish, plus up to 1.53 MB of Swagger UI when
bundled — so committing it should be an explicit act (`git add -f`), not a side effect of
`git add -A`.
