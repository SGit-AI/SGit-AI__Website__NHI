...
PACKS = [
 dict(slug="static-publishing",
  ...
  origin="Authored by an Architect-review agent in the SGit-AI__CLI repo (branch claude/sgit-architect-agent-review-3tl5ti), 17 August 2026; revised twelve times through 19 August under maintainer review — added documents (publish output, API docs, asset origin), the no-target and no-ciphertext-copy publish decisions, two executed tabletop exercises (the one-repo Pages scenario; the publishing pipelines, whose brief was authored in this site's working session), a committed canonical Pages workflow template, and a change-control log recording every revision with its trigger. Status: build spec, ready to implement — Phases 1 and 3 are unblocked.",
  ...
  commit="a7fb3f5", captured="19 August 2026",
  commit_url="https://github.com/SGit-AI/SGit-AI__CLI/commit/a7fb3f5af9f84b0c62024b25e8332c0362b11d50",
  row_date="17–19 Aug 2026 · build spec",
  one_line="Publish a vault to any static host or folder; browse and clone it with no server and no auth. 14 documents, a change-control log, and a committed Pages workflow template — including two executed tabletop exercises.",
  meta_desc="A build-spec pack, readable in-page: the executable dev brief, architecture, commands &amp; UX, flows, invariants &amp; tests, phases, the publish output, published API docs, the asset-origin question, decisions &amp; evidence, two executed tabletop exercises with their pipelines brief, the canonical Pages workflow template, and the pack's change-control log.",
  readme="README.md",
  ...
  docs=[
   ...
   dict(slug="architecture", file="01__architecture.md",
    ...
    summary="The whole feature hangs on one seam: every sgit action already accepts an injected api, so the static transport is a sibling class — read-only GET fan-out over any host or folder — with zero changes to any call site, proven by a running spike. Four methods carry the entire read path (including presigned_read_url, which is on the clone path for every blob over ~4 MB — a size-dependent bug waiting for anyone who skips it). Transport resolution is transparent but never silent: sniffed once, sticky, reported in vault info. As of r9, publish emits the plaintext surface only — the ciphertext store is never copied; the served root is composed at deployment, either co-located (serve the repo itself, zero copies) or assembled by a keyless copy of bare/ into place. r12 lands finding F5 in the transport contract — only an HTTP 404 means absent; connection errors raise loudly, naming the host — and adds the pipeline seam: the CI sequence with the runner's key posture.",
    concepts=[
   ...
   dict(slug="flows", file="03__flows.md",
    ...
    summary="The design as sequence diagrams (rendered below): the baseline publish → serve → browser read, where no server does any of the work; the static clone over plain GETs — verified end to end against a real GitHub Pages host; the fork round trip; and, added in r12, flow 6 — the CI pipeline sequence from checkout through attach, staleness check, republish and compose to deploy. The diagrams are the review surface: each one is also a test cell.",
    concepts=[
   ...
   dict(slug="implementation-phases", file="05__implementation-phases.md",
    ...
    summary="Independently shippable phases, each one PR with green suites and its own acceptance criteria. P1 (static read transport — promote the spike, don't redesign it) and P3 (vault serve) start now: demonstrable value with zero commitment to the publish protocol still being decided; the revisions add P4b (the published API docs, gated on P2) and P9 (sgit vault attach — binding a key to a fresh git checkout, the gap both tabletops hit). Acceptance criteria are concrete down to the trap: add a >4 MB fixture, because small fixtures will not catch the large-blob path.",
    concepts=[
     ("Promote the spike", None, "the P1 design already exists as running code — productionising it is the task"),
     ("Ship value before protocol", "decisions-and-evidence.html", "P1+P3 first is itself one of the fifteen decisions, with a recommendation"),
    ],
    ideas=[
   ...
   dict(slug="decisions-and-evidence", file="06__decisions-and-evidence.md",
    ...
    role="Fifteen open decisions for the maintainer, and the measured evidence that re-scoped the spec",
    summary="The maintainer's file: fifteen decisions (up from six across the 18–19 Aug revisions — 11: plaintext expansion becomes a future sgit vault expand; 12: publish emits the surface only, no ciphertext copy; 13: the canonical three-line repo-side .gitignore guarding key material, emitted by sgit; 14: the sgit vault attach command; 15: the deploy workflow comes from a CLI generator, with the human-readable copy on sgit.ai), each with a recommendation, none blocking P1/P3. And the evidence base that changed the spec: GitHub Pages sends access-control-allow-origin: * by default, so key-on-another-origin moved from 'probably unavailable' to 'supported, asserted at run time'; custody without access structurally requires a manifest, because every filename derives from the read key; object ids are sha256(ciphertext) with random IVs, so two vaults holding the same document share zero object ids — a fork is unlinkable; and Swagger UI measures 2.7× the vault it documents, which reversed decision 7 to a CDN-with-SRI default.",
    concepts=[
   ...
   dict(slug="publish-target", file="07__publish-target.md",
    ...
    role="Revised 19 Aug (three times): publish takes no target, copies no ciphertext, and its folder is plainly committable — the gitignore that matters guards key material",
    ...
    concepts=[
     ...
     ("The gitignore that matters guards keys", None, "the canonical repo-side set is three lines: local/ (live secrets), backups/ (zips can carry the vault key itself as VAULT-KEY), .sg_vault_new/ (a second store, with its own secrets, during a move)"),
   ...
   dict(slug="asset-origin", file="09__asset-origin.md",
    ...
    concepts=[
     ...
     ("A permanent public commitment", "https://pki.sgit.ai/rules/index.html", "URLs baked into folders on hosts you do not control can never change: append-only, never rewritten, indefinitely — the registry-rules discipline applied to an asset bucket"),
    ],
    ideas=[
   ...
   dict(slug="pipelines-brief", file="11__tabletop-brief__publishing-pipelines.md",
    title="11 (brief) — Publishing pipelines: the scenario",
    role="The scenario brief for the second tabletop — authored in the nhi.sgit.ai working session, adopted into the pack with an intake note",
    summary="The brief that scoped the pipelines tabletop, written on the other side of this collaboration: it was drafted in the nhi.sgit.ai site session (the one that maintains these pages), published on this site's briefs/ channel, and adopted into the pack verbatim plus an intake note. §0 states the two ignore systems side by side and pre-registers two findings established by code inspection before any run: the .sg_vault/backups/ key-leak hazard (backup zips can carry the vault key itself), and the missing attach command. Ten steps with a cast of eight — publisher, two CI runners, a fork contributor, two hosts, a reader, a rollback operator — each trap to be walked into deliberately; §5 lists the edits the pack files owed independent of the run, which landed as r10 and r12. The intake note records one deviation: the maintainer removed the publish folder's self-ignore entirely rather than keeping it as a gate.",
    concepts=[
     ("Pre-registered findings", None, "the keyed-backup hazard and the attach gap were established by inspection before the run — the tabletop's job is to confirm them by execution, which it did"),
     ("Traps walked into deliberately", "tabletop-pipelines.html", "the forgotten --visibility, the fork PR without secrets, the keyed backup — each must end as a fix or a decided behaviour, never a surprise left for production"),
     ("Cross-session authorship", None, "written on nhi.sgit.ai, executed in the CLI repo, captured back here — the site working as the public comms channel it documents"),
    ],
    ideas=[
   ...
   dict(slug="tabletop-pipelines", file="11__tabletop__publishing-pipelines.md",
    title="11 — Tabletop (executed): publishing pipelines",
    role="The executed pipelines tabletop: the canonical gitignore proven, the attach drill, the zero-secret pipeline, rollback semantics — findings F5–F7",
    explainer=("What is happening in this document", [
     "This is the pack's <b>second executed tabletop</b> — the form is explained on the <a href='tabletop-github-pages.html'>first one's page</a>: a security-style walkthrough where the agent plays every party and actually runs the commands rather than imagining their output. Here the cast is a publisher, a zero-secret CI runner, a private-vault runner holding a key as a secret, a fork contributor who gets no secrets, a reader, and a rollback operator.",
     "This run is in <b>simulated-hosting mode</b>, at the maintainer's instruction: GitHub, Actions and Pages are stand-ins, marked SIMULATED at every appearance, while everything sgit-shaped — the CLI, the crypto, the server — is real. What simulation cannot measure is not fudged: it is marked NOT MEASURABLE and deferred, and those markers now define exactly what a future real-GitHub run must cover.",
     "Once again the exercise changed the spec it tested. F5: a dead host was indistinguishable from an empty vault, sending the operator toward re-keying when the truth was &ldquo;server down&rdquo;. F6: the attach command's two acceptance criteria were discovered by the shipped clone-mode guard refusing bad states. F7: a workaround applied in the wrong order silently deployed a stale site. And the pre-registered keyed-backup leak from the brief was confirmed by executing it — one keyed backup plus git add -A stages the vault key.",
    ]),
    summary="All ten steps of the brief, run with the real CLI and simulated hosting. Proven executed: the canonical three-line .gitignore (the keyed-backup zip staged under the old ignore, excluded under the new); the attach drill (wrong key refused with nothing written; the workflow file swept into the vault and arriving in the reader's clone — the F1-generalisation policy is now ACCEPTED and demonstrated); the full public pipeline with a zero-secret runner; R3 demonstrated (a forgotten --visibility silently drops the key file — the safe direction); the keyless staleness check catching a forgotten republish from manifest hashes alone; and rollback as git revert — the site follows the repo timeline, not the vault's. New findings F5 (dead host reads as empty vault — only 404 means absent), F6 (attach must be mode-exclusive and schema-exact), F7 (refs-checkout after a successful push destroys the head; safe ordering now in the committed workflow template), plus a sharpened F2. The closing table records who held what and for how long — the private-vault runner is the one key-holding service in the whole model.",
    concepts=[
     ("F5: a dead host reads as an empty vault", None, "connection failure was indistinguishable from 404-absent, diagnosing 'no named ref' when the truth was 'host down' — P1 acceptance now requires connection errors to raise loudly, naming the host"),
     ("The workflow template, hardened by breaking", "src/templates/github-pages.yml", "F7's safe ordering (restore refs before sgit operations, commit after push) and the fork-guard glob fix live as comments in a committed artifact — decision 15's generator will emit it"),
     ("The site follows the repo timeline", None, "rollback is git revert: surface and store roll together because both live in one git commit; the SG server's head still shows v2 — a published site is a projection of a git commit"),
     ("The one key-holding service", None, "the closing table: the private-vault runner holds a read key for the job lifetime — scoped, ephemeral, read-only; the public runner held only what the world already had"),
    ],
    ideas=[
