...
PACKS = [
 dict(slug="static-publishing",
  ...
  origin="Authored by an Architect-review agent in the SGit-AI__CLI repo (branch claude/sgit-architect-agent-review-3tl5ti), 17 August 2026; revised eight times through 19 August under maintainer review — three added documents (publish output, API docs, asset origin), the no-target publish decision, an executed end-to-end tabletop exercise, and a change-control log recording every revision with its trigger. Status: build spec, ready to implement — Phases 1 and 3 are unblocked.",
  ...
  commit="a3b4ccf", captured="19 August 2026",
  commit_url="https://github.com/SGit-AI/SGit-AI__CLI/commit/a3b4ccfbf432a467921820cc3a85bd1f6f179489",
  ...
  docs=[
   ...
   dict(slug="implementation-phases", file="05__implementation-phases.md",
    ...
    concepts=[
     ...
     ("Ship value before protocol", "decisions-and-evidence.html", "P1+P3 first is itself one of the eleven decisions, with a recommendation"),
    ],
    ideas=[
   ...
   dict(slug="changelog", file="CHANGELOG.md",
    ...
    role="r0–r8, newest first: every spec-editing commit adds an entry — what changed, the trigger, and which decisions moved",
    summary="The pack has reversed load-bearing decisions more than once — healthy, the log says, but only if the reversals are legible in one place. Eight revisions in three days, each entry naming its trigger (almost always a one-line maintainer question: 'why do you need the vault.html file?', 'publish ./site should not be allowed'), what changed, and which numbered decisions moved. The latest entry, r8, is the log acting on itself: the R1 contradiction fixed by making plaintext expansion a future sgit vault expand command (decision 11), dead flags removed, manifest entries gaining a per-object sha256 because the tabletop's keyless mirror could content-verify only 12 of 18 objects, and the visibility-downgrade warning added because a fresh CI clone resolves to bare and would silently drop the key file. Updating this file is now part of a developer agent's definition of done.",
    concepts=[
